...
预训练样本可以一次性生成，存储为 .pt 文件，按日期命名（如 sample_20230401.pt）。训练时通过自定义 Dataset 加载。

生成预训练样本的关键约束：

特征截面：对每一天，按当天实际成分股市值降序排列，填充 500 个槽位（不足时补零并记录 mask）。
标准化：每个特征在截面上做稳健 Z-score（减去中位数，除以四分位距），或使用历史分位数归一化，避免使用未来信息。
教师权重：使用决策日 T 的已知市值计算 w_i = MarketCap_T(i) / Σ_j MarketCap_T(j)，复制到未来 5 天，即 teacher_weights[k,:] = w for all k。
2.3 GRPO 后训练数据准备
...
每日收盘后，生成一个新的 .pt 样本文件，放入 ./daily_samples/ 目录。文件名建议包含日期，如 sample_20250728.pt。该样本包含完整的 6 个字段（teacher_weights 可选，因为在线更新仅用 GRPO 奖励）。DailyUpdate.py 会自动加载该目录下最近 N 天的文件（由 recent_days 配置决定）。
...
bash
python PreTrain.py
模型将保存在 ./checkpoints/best_pretrain.pth。
...
bash
python GRPOTrain.py
微调后的模型保存为 ./checkpoints/best_grpo.pth。
...
当模型规模大，MoE 专家数量多，单卡无法容纳全部专家参数时，需将不同专家放在不同 GPU 上，即专家并行。
